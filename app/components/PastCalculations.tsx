...
export default function PastCalculations({ refreshTrigger }: PastCalculationsProps) {
  ...
  const saveEdit = (index: number) => {
    ...
    const updatedEntry: SavedInvestment = { name: editForm.name || 'Untitled', amount: principal, rate, startYear: start, endYear: end, data };
  ...
  return (
    <div>
          ...
          <div key={i} className={`border rounded p-3 ${selected.includes(i) ? 'border-blue-500 bg-blue-50' : 'border-gray-200'}`}>
            {editingIndex === i ? (
              ...
              <div className="flex items-center gap-2">
                <p onClick={() => toggleSelect(i)} className={`font-semibold flex-1 cursor-pointer select-none ${selected.includes(i) ? 'text-blue-700' : 'text-gray-900'}`}>
                  {inv.name}
                </p>
      ...
      <button onClick={() => setComparing(true)} disabled={selected.length !== 2} className="mt-4 bg-purple-600 text-white px-4 py-2 rounded hover:bg-purple-700 disabled:bg-gray-300 disabled:cursor-not-allowed">
        Compare
      </button>
